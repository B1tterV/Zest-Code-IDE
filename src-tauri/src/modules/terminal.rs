use portable_pty::{native_pty_system, Child, CommandBuilder, PtySize, MasterPty}; // Добавили MasterPty
use std::{
    io::{Read, Write},
    sync::{Arc, Mutex},
    thread,
...
pub fn spawn_terminal(
    ...
    thread::spawn(move || {
        ...
        println!("PTY Process for {} CLOSED (Reader loop ended)", id_for_thread);
    });
...
pub fn write_to_terminal(id: &str, data: &str, state: &TerminalState) -> Result<(), String> {
    ...
    if let Some(instance) = lock.get_mut(id) {
        instance.writer.write_all(data.as_bytes()).map_err(|e: std::io::Error| e.to_string())?;
        instance.writer.flush().map_err(|e: std::io::Error| e.to_string())?;
    }
    Ok(())
}

pub fn resize_terminal(id: &str, cols: u16, rows: u16, state: &TerminalState) -> Result<(), String> {
    let mut lock = state.0.lock().unwrap();
    if let Some(instance) = lock.get_mut(id) {
